(function () {
  ...
  function normalizeDriveThumbnail(url) {
    ...
    if (!id) return url;
    return `https://drive.google.com/thumbnail?id=${id}&sz=w64`;
  }
  ...
  function renderPosts(posts, query) {
    ...
    const withIds = posts.map((p, i) => ({
      ...p,
      id: p.id != null && !Number.isNaN(Number(p.id)) ? Number(p.id) : (total - i),
    }));
